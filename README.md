...
```
  Taxfix Loop is running at http://127.0.0.1:5173
    /            landing page
    /app.html    the three-screen prototype

  Straight to a moment:
    /app.html?replay=1                  watch the agents work
    /app.html?answer=scared#ask         humour switches off
    /app.html?repair=mostly-freelance   the repair loop, answered
...
| Page | What it is |
|---|---|
| `web/index.html` | **Landing page** — the story, the shift, the AI team, the rejected joke. Its hero contains a **live, scaled instance of the real prototype** in a phone frame, not a screenshot. |
| `web/app.html` | **The prototype** — three screens: Today, AI Team, Ask. |

Both link `web/tokens.css`, so the palette cannot drift between them. The landing
page forwards any prototype deep link (`?replay`, `?repair`, `?answer`, `#team`,
`#ask`) through to `app.html`, so old links keep working.
...
Both pages are mobile-first. The prototype sits inside `env(safe-area-inset-*)`
padding so it clears a notch and the home indicator, and below 430px the readiness
ring moves above its explanation instead of squeezing the copy into a narrow
column. The landing page drops its phone mock below 560px, where it would be
...
```
app.html#today  app.html#team  app.html#ask
app.html?repair=mostly-freelance   # today screen, repair loop already answered
app.html?answer=scared             # the sensitive-mode answer
app.html?replay=1                  # the AI team replays its work
```

These also work from the root — `/` forwards them into the app.
...
**0 · The landing page (10s).** Open on `/`. Read the headline aloud — *"Feel ready
before tax season arrives"* — and point at the phone: "that is the real prototype
running, not a recording." Then click **Open the prototype**.
...
web/
  index.html  landing.css            the landing page (live phone mock of app.html)
  app.html  styles.css  app.js       the three-screen prototype
  tokens.css                         shared palette — linked by both pages
